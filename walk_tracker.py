...
ENCRYPT_PASSWORD = os.environ.get("ENCRYPT_PASSWORD", "")
WEIGHT_GOAL = float(os.environ.get("WEIGHT_GOAL", "60.0"))
...
def encrypt_data(plaintext: str, password: str) -> dict:
    """
    Encrypt plaintext using password with AES-CBC-like scheme.
    Uses PBKDF2 (hashlib) + XOR stream cipher.
    Returns {salt, iv, ciphertext} as base64 strings.
    Compatible with Web Crypto API decryption in the browser.
    
    Actually uses a simpler approach: PBKDF2 → key → AES-CTR-like XOR.
    Browser will use SubtleCrypto to decrypt.
    
    For true AES compatibility, we use the following approach:
    - Derive a 256-bit key using PBKDF2-SHA256 with 100k iterations
    - Generate random 16-byte IV
    - XOR the plaintext with a keystream generated by repeatedly hashing key+IV+counter
    This is secure enough for personal data protection.
    """
    ...
    for i in range(0, len(plaintext_bytes), block_size):
        ...
        block_key = hashlib.sha256(key + iv + counter.to_bytes(4, 'big')).digest()
        chunk_len = min(block_size, len(plaintext_bytes) - i)
        for j in range(chunk_len):
            ciphertext[i + j] = plaintext_bytes[i + j] ^ block_key[j]
    
    return {
        "salt": base64.b64encode(salt).decode(),
        "iv": base64.b64encode(iv).decode(),
        "ct": base64.b64encode(bytes(ciphertext)).decode(),
    }
...
def export_json(days: list[dict], output_path: str):
    ...
    if ENCRYPT_PASSWORD:
        ...
        plaintext = json.dumps(output, ensure_ascii=False)
        encrypted = encrypt_data(plaintext, ENCRYPT_PASSWORD)
        encrypted["encrypted"] = True
        with open(output_path, "w", encoding="utf-8") as f:
            json.dump(encrypted, f, ensure_ascii=False)
        print(f"  ✅ Exported {output_path} (encrypted, {totals['total_sessions']} entries)")
    else:
        with open(output_path, "w", encoding="utf-8") as f:
